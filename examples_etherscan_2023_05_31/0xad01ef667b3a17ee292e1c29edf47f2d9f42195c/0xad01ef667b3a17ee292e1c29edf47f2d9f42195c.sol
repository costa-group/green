...
contract LampsPaymentSplitter {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address payable constant TREASURY = payable(0x0bA6585009c2E275CdE3Ede4Ab086d2c8b71b155);
    address payable constant UWU_LABS = payable(0x354A70969F0b4a4C994403051A81C2ca45db3615);
    ...
    function withdrawToken(address token) public {
        if (token == address(0)) {
            ...
            sendValue(UWU_LABS, percentOfTotal(totalBalance, 0.10 gwei)); // 0.5%/5% - 10% of royalties
            sendValue(TREASURY, address(this).balance); // 4.5%/5% - 90% of royalties
        } else {
            uint256 totalBalance = IERC20Upgradeable(token).balanceOf(address(this));
            IERC20Upgradeable(token).safeTransfer(UWU_LABS, percentOfTotal(totalBalance, 0.10 gwei)); // 0.5%/5% - 10% of royalties
            IERC20Upgradeable(token).safeTransfer(TREASURY, IERC20Upgradeable(token).balanceOf(address(this))); // 4.5%/5% - 90% of royalties
        }
    }
