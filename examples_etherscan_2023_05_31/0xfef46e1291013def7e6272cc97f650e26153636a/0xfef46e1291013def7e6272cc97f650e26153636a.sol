...
interface IAzukiForge {
    function _nftTokenForges(uint256 tokenId) external view returns (address);
}

contract AzukiApes is ERC721, DefaultOperatorFilterer, Ownable {
    IAzukiForge public azukiForge;
    address private AASC = 0x813b5c4aE6b188F4581aa1dfdB7f4Aba44AA578B;
    address private AFORGE = 0xc4Df0F70A590D01e6cA3B15284b2001c0c60c695;

    uint256 private maxSupply = 3333;
    bool public claimIsActive = true;

    string private _bTokenURI =
        "https://ipfs.io/ipfs/QmagJA4BkXLCsQRtKB74qGxYVzaqWAnqiMNa2dPRmFBrQt/";

    // NOTE: Map old AASC to a new Azuki Ape
    mapping(uint => uint256) public apeClaimed;

    constructor() ERC721("Azuki Apes", "AzukiApe") {
        azukiForge = IAzukiForge(AFORGE);
    }
    ...
    function claimApes(uint256[] memory theTokenIDs) public {
        ...
        uint256 senderBalance = IERC721(AASC).balanceOf(msg.sender);
        ...
        for (uint256 i; i < numTokens; i++) {
            ...
            if (apeClaimed[tokenId] == 0) {
                if (IERC721(AASC).ownerOf(tokenId) != msg.sender) {
    ...
    function claimForgedApes(uint256[] memory theTokenIDs) external {
        ...
        for (uint256 i; i < numTokens; i++) {
            ...
            if (apeClaimed[tokenId] == 0) {
                if (azukiForge._nftTokenForges(tokenId) != msg.sender) {
