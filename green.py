...
from gasol_optimizer.sfs_generator.parser_asm import (
    parse_asm,
...
from gasol_optimizer.verification.sfs_verify import (
    verify_block_from_list_of_sfs,
    are_equals,
)
from gasol_optimizer.solution_generation.optimize_from_sub_blocks import (
    rebuild_optimized_asm_block,
)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["baseref", "offset", "jump-origin"],
    )
    parser.add_argument(
        "-aliasing-info",
        "--aliasing-info",
        help="Executes optimization on blocks obtained by memory analysis.",
        action="store_true",
    )
    parser.add_argument(
        "-non-aliasing-disabled",
        "--non-aliasing-disabled",
        help="It only uses the information of aliasing.",
        action="store_true",
    )
    parser.add_argument(
        "-useless-info",
        "--useless-info",
        help="Uses useless info from memory analysis.",
        action="store_true",
    )
    parser.add_argument(
        "-context-info",
        "--context-info",
        help="Uses context info from memory analysis.",
        action="store_true",
    )
    parser.add_argument(
        "-compact-clones",
    ...
    parser.add_argument(
        "-ccfg",
        "--collapse-cfg",
        help="collapses inconditional jumps",
        choices=["yes", "no"],
    )
    ...
    output.add_argument(
        "-o",
        help="Path for storing the optimized code",
        dest="output_path",
        action="store",
    )
    ...
    log_generation = parser.add_argument_group(
        "Log generation options", "Options for managing the log generation"
    )

    log_generation.add_argument(
        "-log",
        "--generate-log",
        help="Enable log file for verification",
        action="store_true",
        dest="log",
    )
    log_generation.add_argument(
        "-dest-log", help="Log output path", action="store", dest="log_stored_final"
    )
    log_generation.add_argument(
        "-optimize-from-log",
        dest="log_path",
    ...
    basic = parser.add_argument_group(
        "Max-SMT solver general options",
    ...
    basic.add_argument(
        "-solver",
        "--solver",
        help="Choose the solver",
        choices=["z3", "barcelogic", "oms"],
        default="oms",
    )
    basic.add_argument(
        "-tout",
        ...
        action="store",
        type=int,
        help="Timeout in seconds. By default, set to 10s per block.",
        default=10,
    )
    basic.add_argument(
        "-direct-tout",
    ...
    basic.add_argument(
        ...
        "--greedy",
        dest="greedy",
        help="Uses greedy directly to generate the results",
        action="store_true",
    )

    blocks = parser.add_argument_group(
        "Split block options",
        "Options for deciding how to split blocks when optimizing",
    )

    blocks.add_argument(
        "-storage",
        "--storage",
        help="Split using SSTORE, MSTORE and MSTORE8",
        action="store_true",
    )
    blocks.add_argument(
        "-partition",
        "--partition",
        help="It enables the partition in blocks of 24 instructions",
        action="store_true",
    )

    hard = parser.add_argument_group(
        "Hard constraints", "Options for modifying the hard constraint generation"
    )

    hard.add_argument(
        "-memory-encoding",
        help="Choose the memory encoding model",
        choices=["l_vars", "direct"],
        default="direct",
        dest="memory_encoding",
    )
    hard.add_argument(
        "-no-simplification",
        "--no-simplification",
    ...
    soft = parser.add_argument_group(
        "Soft constraints", "Options for modifying the soft constraint generation"
    )
    group_gasol = soft.add_mutually_exclusive_group()
    group_gasol.add_argument(
        "-size",
        "--size",
        action="store_true",
    ...
    additional = parser.add_argument_group(
    ...
    additional.add_argument(
        "-order-conflicts",
        action="store_false",
        dest="order_conflicts",
        help="Disable the order among the uninterpreted opcodes in the encoding",
    )

    ml_options = parser.add_argument_group(
        "ML Options", "Options to execute the different ML modules"
    )
    ml_options.add_argument(
        "-bound-model",
        "--bound-model",
        action="store_true",
        dest="bound_select",
...
def analyze_solidity(input_type="solidity"):
    ...
    if input_type == "solidity":
        helper = InputHelper(
            ...
            tmp_path=global_params.tmp_path,
        )
    ...
    if len(inputs) == 1:
        ...
        try:
            ...
            if symExec.opt_blocks != None:
                optimized_blocks[
                    symExec.opt_blocks.get_contract_name()
                ] = symExec.opt_blocks
    ...
    if symExec.file_info != {}:
        for k in symExec.file_info:
            ...
            r += (
                ...
                + str(info["num_blocks"])
                + ";"
                + str(info["num_blocks_cloning"])
                + ";"
                + str(info["optimizable_blocks"])
                + ";"
                + str(info["memory_blocks"])
                + ";"
                + str(info["memory_blocks2"])
                + ";"
                + str(info["storage_blocks"])
                + ";"
                + str(info["time"])
            )
            print(r)
...
def run_gasol(
    ...
    for old_block in blocks:
        ...
        asm_block, _, statistics_csv = gasol_main.optimize_asm_block_asm_format(
            old_block, timeout, parsed_args, dep_information, opt_info
        )
        ...
        if gasol_main.equal_aliasing:
            print(
                ...
                + "_"
                + str(block_id)
                + " FILTERED WITH EQUAL SFS WITH AND WITHOUT HEAP ANALYSIS INFORMATION"
            )
            return 0
        ...
        if not has_info:
            eq, reason = gasol_main.compare_asm_block_asm_format(
                old_block, asm_block, parsed_args, dep_information, opt_info
            )
    ...
    if parsed_args.backend:
        ...
        print(
            "\n".join(
                [asm_block.to_plain_with_byte_number() for asm_block in asm_blocks]
            )
        )
    ...
    if parsed_args.backend:
        ...
        if has_info:
            has_memory = (dep_information.get_equal_pairs_memory() != []) or (
                dep_information.get_nonequal_pairs_memory() != []
            )
            has_storage = (dep_information.get_equal_pairs_storage() != []) or (
                dep_information.get_nonequal_pairs_storage() != []
            )
            has_useless = opt_info["useless"] and (
                dep_information.get_useless_info() != []
            )
        ...
        greenres = [
            args.source + "_" + contract_name + "_" + str(block_id),
            args.source,
            contract_name,
            block_id,
            real_timeout,
            ...
            instructions,
            opt_instructions,
            gasol_main.previous_gas,
            gasol_main.previous_size,
            gasol_main.prev_n_instrs,
            gasol_main.new_gas,
            gasol_main.new_size,
            gasol_main.new_n_instrs,
            dif_gas,
            dif_size,
            dif_n_instrs,
            has_memory,
...
def final_file_names(parsed_args, cname, block):
    ...
    if parsed_args.log_stored_final is None:
        if parsed_args.output_path is None:
            ...
            log_file = (
                parsed_args.output_path + input_file_name + "_" + str(block) + ".log"
            )
    else:
...
def run_gasol_test(dep_information={}):
    ...
    for old_block in blocks:
        asm_block, _, statistics_csv = gasol_main.optimize_asm_block_asm_format(
            old_block, timeout, parsed_args, dep_information, opt_dict
        )
...
if __name__ == "__main__":
    ...
    for c in opt_blocks:
        ...
        for b in blocks:
            ...
            opt_dict["non_aliasing_disabled"] = (
                args.non_aliasing_disabled and args.aliasing_info
            )
            ...
            if not optimize:
                ...
                if (
                    opt_dict["useless"]
                    and opt_dict["dependences"]
                    and opt_dict["context"]
                ):
                    ...
                    if (
                        blocks[b].has_context_info()
                        and blocks[b].get_useless_info() != []
                    ):
                    ...
                    if (
                        blocks[b].has_dependences_info()
                        and blocks[b].has_context_info()
                    ):
